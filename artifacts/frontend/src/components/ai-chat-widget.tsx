...
export default function AIChatWidget() {
  ...
  async function sendMessage(text?: string) {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: t("aiChat.error") },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  const quickReplies = [
    t("aiChat.quickReply1"),
    t("aiChat.quickReply2"),
    t("aiChat.quickReply3"),
    t("aiChat.quickReply4"),
  ];
      ...
      {open && (
        ...
        >
          ...
          <div className="px-5 py-4 flex items-center gap-3 shrink-0" style={{ background: "#1E2A3A" }}>
            ...
            <div>
              <p className="text-white font-semibold text-sm">{t("aiChat.assistantName")}</p>
              <div className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-green-400"></div>
                <p className="text-green-400 text-xs">{t("aiChat.online")}</p>
              </div>
            </div>
